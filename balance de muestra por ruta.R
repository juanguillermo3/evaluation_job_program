    ...
    Sesiones_ruta_empleabilidad %>% 
      ...
      
      {
        ...
        data.frame(
          ...
          logro_universitario= 
            round(mean(data_de_participantes$estudios %in% c("Universitaria", 
                                                             "Maestría",
                                                             "Especialización", 
                                                             "Doctorado" )*100, 
                       na.rm=TRUE), 3),
          
          oferente_no_nacional= 
            round(mean(data_de_participantes$tipo_doc %in% c("Cédula de Extranjeria",
                                                             "Documento Nacional de Identificación",
                                                             "Pasaporte",
                                                             "Permiso Especial de Permanencia"
            ), na.rm=TRUE)*100, 3),
          
          desempleo_al_registro= 
            round(mean(data_de_participantes$laboral_states %in% c("Cesante por Emergencia Sanitaria",
                                                                   "Desempleado"), na.rm=TRUE)*100, 3),
...
#
Sessions_df.prestador_es_minuto_de_dios.focalizacion_de_tratamiento_balance() %>% View()
