...
when_placements(limits=c(FECHA_INICIO_COVID,FECHA_INICIO_COVID+365*(1))) %>% nrow()
when_placements(limits=c(FECHA_INICIO_COVID-365*(1),FECHA_INICIO_COVID)) %>% nrow()
#
...
covid_design.count_outcomes_in_periods=
  function(
    ...
    covid_design=
      db.read_unlabelled_sample() %>% 
      
      dplyr::mutate(
        fecha=as.Date(ultima_actividad)
      ) %>%
    ...
    covid_design %>%
      #
      split(.$"actividad_post_covid") %>% 
      #
      lapply(function(sub_df){
        
        #
        period_wise_limits=
        {if(sub_df$actividad_post_covid[1]=="pre-covid"){
    ...
    covid_design %>% 
      dplyr::left_join( new_placements_df) %>%
      dplyr::mutate(
        phase_placements=ifelse(is.na(phase_placements), 0, phase_placements  )
      ) %>%
      #
      {
...
outcomes.labelled_sample_for_covid_design=
  function(
    ...
    treatments=grep("al_menos",names(covid_design.count_outcomes_in_periods()), value=TRUE)
  ){
    #
    new_df=outcomes 
    #
    new_df$dd_post_covid=as.numeric(new_df$actividad_post_covid=="post-covid")
    for(some_treatments in treatments){
      #
      #
      tr_temp_var=sprintf("dd_Treatment_%s", 
                          stringr::str_extract(some_treatments, "[0-9]$")
                          )
      new_df[[tr_temp_var]]=new_df[[some_treatments ]]
      #
      #
      new_df[[ sprintf("dd_Post_CovidXTreatment_%s",
                       stringr::str_extract(some_treatments, "[0-9]$")
                       ) ]]= 
...
Dataframe.share_with_app(df,
                         label="covid_paula_design_eval_sample",
                         app_route =LABELLED_SAMPLE_HOME )


covid_sample.genderized_overview=function(
  covid_sample=outcomes.labelled_sample_for_covid_design() 
){
  covid_sample %>% 
    people_with_ids_df.genderdize() %>% 
    split(paste(.$sexo, .$actividad_post_covid, sep="and")) %>%
    lapply(function(sub_df){
      data.frame(
        gender=sub_df$gender[1],
        registro_post_covid=sub_df$actividad_post_covid[1],
