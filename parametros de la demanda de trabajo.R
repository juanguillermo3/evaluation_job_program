...
db.raw_text_from_vacancies.cache=db.raw_text_from_vacancies()

#
#
Texto_vacantes_df.parametros_contrato=function(
  Texto_vacantes_data=db.raw_text_from_vacancies.cache
){
  ...
  Texto_vacantes_data %>%
    ...
    dplyr::mutate(
      codigo_vacante=stringr::str_extract(texto_vacante_estandarizado, "[[0-9][-]]{5,20}")
    ) %>%
    dplyr::mutate(
...
#
#
Texto_vacantes_df.persistir_tabla_demanda_de_trabajo=function(
  Texto_vacantes_df=Texto_vacantes_df.parametros_contrato()
){
  #
  setwd(DEMANDA_DE_TRABAJO_HOME)
  #
  write.csv(Texto_vacantes_df, "parametros vacantes.csv", fileEncoding = "UTF-8")
}
#
Texto_vacantes_df.persistir_tabla_demanda_de_trabajo()
