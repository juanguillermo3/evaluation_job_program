...
#
#
Participation_records.model.curate.normalize=function(
...
#
Participation_records.model.curate.normalize() %>% View()

#
#
Ruta_de_empleabilidad_df.Personal_session_records=function(
  Ruta_de_empleabilidad_df=Participation_records.model.curate.normalize()
){
  session_records=
...
Participacion_en_actividades.modelo_participaciones_normalizado() %>%
  Dataframe.share_with_app(
    label="actitivites_dataset",
    app_route =ACTIVITIES_HOME )
...
participaciones_df.en_el_tiempo(
  ...
  labs(
    title = "Actividades de intermediación laboral",
