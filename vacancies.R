...
db.read_vacancies=function(){
  setwd(VACANTES_HOME)
  readxl::read_excel("vacantes.xlsx")
}
db.read_vacancies() %>% View()


#
vacancies_over_time.plot=function(
  
  vacancies=db.read_vacancies(),
  ...
  vacancies %>% 
    ...
    dplyr::select(
      c("codigo_vacante", "num_vacantes", "fecha_publicacion")
    ) %>% 
    
    na.omit() %>%
    
    dplyr::filter(
      num_vacantes<quantile(num_vacantes, .99)
    ) %>% 
    
    dplyr::mutate(
      fecha=as.Date(fecha_publicacion)
    ) %>% 
    mutate(
      fecha= as.Date(fecha),
    ...
    #
    split(paste(.$periodo, sep="x")) %>%   
    lapply(function(sub_df){
      sub_df=sub_df %>% na.omit() 
      data.frame(
        size=sum(sub_df$num_vacantes),
    ...
    dplyr::mutate(
      fase= 
        ifelse(
        as.Date(periodo)>FECHA_INICIO_COVID,
...
vacancies_over_time.plot()+
  labs(
    subtitle="Nuevos puestos de trabajo registrados por trimestre"
  )+
  ylab("# puestos")

#
vacancies.size_and_text_and_time=function(
  vacancies=db.read_vacancies()
){
  vacancies %>%
    dplyr::transmute(
      ...
      texto_estandarizado=z_vacante,
      fecha_publicacion=fecha_publicacion
    ) %>% na.omit()
}
vacancies.size_and_text_and_time() %>% View()

#
technologies_in_curriculums.list_tech.filter=function(
  vacancies_text=vacancies.size_and_text_and_time(),
  min_freq=3,
  max_docs_prop=.2,
  technologies_in_curriculums=ToolsxType
){
  technologies_in_curriculums %>%
  lapply(function(tools){
    tools %>% 
      dplyr::mutate(
      prop_ocurrence=
        sapply(tool, function(tool_){
          stringr::str_detect(
            vacancies_text$texto_estandarizado, as.character(tool_)
          ) %>% mean()
        })
      ) %>%
      
      dplyr::filter(frequency>=min_freq) %>%
      dplyr::filter(prop_ocurrence<=max_docs_prop) 
  })
}
technologies_in_curriculums.list_tech.filter()

#
vacancies_text.label_tools=function(
  vacancies_text=vacancies.size_and_text_and_time(),
  ToolsxType=technologies_in_curriculums.list_tech.filter(),
  new_event=FALSE,
  cache_name="labelled_vacancies_cache"
){
  ...
  names(ToolsxType) %>%
    lapply(function(tool_type){
      compound_regex=paste(paste( "(", 
                                  ToolsxType %>% 
                                    Dataframe.fetch(tool_type) %>%
                                    dplyr::mutate(tool=as.character(tool)) %>%
                                    dplyr::filter(nchar(tool)>=5) %>%
                                    #head(15) %>%
                                    Dataframe.fetch("tool"), ")", sep=""), collapse="|")
      new_df=vacancies_text
      new_df[[tool_type]]=grepl(compound_regex, new_df$texto_estandarizado)
      new_df
    }) %>%
    Reduce(f=function(x,y){dplyr::left_join(x,y)},x=.) %>%
...
vacancies_text.label_tools(new_event = TRUE) %>% View()

df=vacancies_text.label_tools(new_event = FALSE) 
#
Dataframe.share_with_app(
  df,
  "vacantes_con_etiqueta_tecnologia.xlsx",
  TECNOLOG虯S_HOME
)

setwd( TECNOLOG虯S_HOME)
write.csv2(df, "vacantes_con_etiqueta_tecnologia.csv")



#
#  
labelled_vacancies_over_time.plot_information_technologies=function(
  
  labelled_vacancies_over_time=vacancies_text.label_tools(),
  ...
  labelled_vacancies_over_time %>% 
    ...
    lapply(function(sub_df){
      ...
      data.frame(
        ...
        Text_processors_=sum(sub_df$tech_procesador_de_texto*(sub_df$num_vacantes), na.rm=TRUE),
        Spreadsheets_=sum(sub_df$tech_hojas_de_calculo*(sub_df$num_vacantes), na.rm=TRUE),
        Multimedia_=sum(sub_df$tech_multimedia*(sub_df$num_vacantes), na.rm=TRUE),
        Specialized_Software_=sum(sub_df$tech_otros*(sub_df$num_vacantes), na.rm=TRUE),
    ...
    dplyr::mutate(
      is_post_covid= 
        ifelse(as.Date(periodo)>FECHA_INICIO_COVID, "post-covid", "pre-covid")
    ) %>%
    
    tidyr::pivot_longer(
      cols= grep(names(.), pattern="_$"),
      names_to = "Tipo_de_herramienta",
      values_to= "num_menciones",
    ) %>%
    ...
    ggplot(aes(x=periodo,
               y=num_menciones, 
               group=Tipo_de_herramienta,
               colour=Tipo_de_herramienta,
               fill=Tipo_de_herramienta,
               shape=Tipo_de_herramienta,
               ))+
    geom_point(
      size=3
    )+
    ...
    #facet_grid(cols = vars(is_post_covid),  scales = "free_x" )+
    theme_stata()+
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=8))
}
labelled_vacancies_over_time.plot_information_technologies() +
  labs(
    subtitle="Menciones a tecnolog韆s de la informaci髇 (en vacantes), por trimestre"
  )+
  theme(legend.key.size = unit(.5, 'cm'), #change legend key size
        legend.key.height = unit(.5, 'cm'), #change legend key height
        legend.key.width = unit(.5, 'cm'), #change legend key width
        legend.title = element_text(size=14*.5), #change legend title font size
        legend.text = element_text(size=10*.7))+
  ylab("# menciones")


#  
labelled_vacancies.covid_factor=function(
  
  labelled_vacancies_over_time=vacancies_text.label_tools(),
  window_size_in_months=6
  ...
  labelled_vacancies_over_time %>% 
    ...
    dplyr::mutate(
      is_post_covid=ifelse(fecha>FECHA_INICIO_COVID,
                           "post-covid", "pre-covid")
    ) %>%  
    dplyr::filter(
      abs(fecha-FECHA_INICIO_COVID)<window_size_in_months*30
    ) %>% 
    #Dataframe.fetch("fecha") %>%  summary() 
    
    split(.$is_post_covid) %>%
    lapply(function(sub_df){
      data.frame(
        fase_covid=dplyr::first(sub_df$is_post_covid),
        
        min_date=min(sub_df$fecha),
        max_date=max(sub_df$fecha),
        
        size=sum(1*(sub_df$num_vacantes), na.rm=TRUE),
    
        Text_processors=sum(sub_df$tech_procesador_de_texto*(sub_df$num_vacantes), na.rm=TRUE),
        Spread_sheets=sum(sub_df$tech_hojas_de_calculo*(sub_df$num_vacantes), na.rm=TRUE),
        Multimedia=sum(sub_df$tech_multimedia*(sub_df$num_vacantes), na.rm=TRUE),
        Specialized_Software=sum(sub_df$tech_otros*(sub_df$num_vacantes), na.rm=TRUE)
      )
    }) %>%
    bind_rows() %>%
    ...
    Dataframe.totalize(
      lambda=function(col){
        ...
        tryCatch(
          {
            round(dplyr::last(col)/dplyr::first(col),3)
          },
        error = 
          {
            function(e){NA}
...
covid_factor_table=labelled_vacancies.covid_factor()
  
Dataframe.share_with_app(
  covid_factor_table,
  label="ccvid factor",
  app_route = VACANTES_HOME
)
...
#
#
labelled_vacancies.covid_factor()
...
Digital_skills_df=
  Virtual_profiles.Virtual_profiles_df.Ensemble_features(
    ...
    feature_markers = c(
      ...
      "Hoja de c谩lculo [(]Ejemplo[:] Excel[)](.){2}Herramienta",
      "Otros(.){2}Herramienta",
...
#
# puestos de trabajo publicados a trav茅s del tiempo
#
  ...
  Vacantes_df %>% 
    ...
    #
    {
      ggplot(., aes(x=year_month, y=num_vacantes, group=fase_covid))+
        ...
        labs(title="Vacantes a trav茅s del tiempo",
             subtitle="N煤mero de vacantes publicadas, por mes",
             caption=sprintf("sobre %s vacantes con intermadiaci贸n del Miuto de Dios",
                             sum(.$num_vacantes)))+
  ...
  Vacantes_df.factor_incremento_covid_global(
    ...
    ) %>%
    {
      ggplot(., aes( group=fase_covid))+
        ...
        labs(title="Evoluci贸n de las vacantes seg煤n fase del covid",
             subtitle="N煤mero de vacantes seg煤n fase",
             caption=
...
Vacantes_df.Modelo_de_fases=function(
  ...
  Vacantes_df %>% 
    ...
    dplyr::filter(!(is.na(.$"Fecha Publicaci贸n"))) %>%
    dplyr::transmute(
      cargo=.$"Cargo",
      num_vacantes=.$"Puestos de Trabajo",
      fecha_publicacion=as.Date(.$"Fecha Publicaci贸n", format="%d/%m/%Y") 
    ) %>%
    #
    # (1) segmentar segun la fase del covid
    #
...
Vacantes_df.factor_incremento_covid.plot=function(
  ...
  Vacantes_df.factor_incremento_covid(
    ...
    dplyr::mutate(
      ...
      list(
        .$"Relacionada con TIC",
        dplyr::sample_n(.$"Gen茅ricas", non_highligted_sample_size),
        dplyr::filter(.$"Gen茅ricas", cargo=="Todos los cargos")
      )
    } %>% bind_rows() %>%
    dplyr::arrange(
      factor_covid*(1)
...
#
mean(unlist(procesosXCargo) %in% df_colocados$"C贸digo Vacante")
...
Vacantes_df.per_semester.plot_cargos=function(
  ...
  Vacantes_df$"Fecha Publicaci贸n"=as.Date(Vacantes_df$"Fecha Publicaci贸n", format="%d/%m/%Y") 
    ...
    df=
      ...
      dplyr::filter(
        (Vacantes_df$"Fecha Vencimiento">=some_semester$min_date) & 
          (Vacantes_df$"Fecha Publicaci贸n"<=some_semester$max_date) &
...
Vacantes_df.plot_puestos_de_trabajo.por_tipo_contrato=function(
  ...
  vp=
    lapply(Y_M, function(some_ym){
      ...
      Vacantes_df %>% 
        dplyr::filter(
          (some_ym$min_date<=Vacantes_df$"Fecha Publicaci贸n") & 
            (Vacantes_df$"Fecha Publicaci贸n"<=some_ym$max_date) &
            Vacantes_df$"Estado Actual"=="Publicada"
        ) %>% 
        {
